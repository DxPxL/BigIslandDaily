...
def wiki_index(request: HttpRequest) -> HttpResponseRedirect:
    """
...
def edit(
    ...
    try:
        ...
        if rev_id:
            ...
            if rev.pk != rev_specific.pk:
                ...
                initial = {
                    "content": rev.content,
                    "message": ('Reverted to "%s"') % rev.message,
                }
    ...
    # Note that it's not saved here
        ...
        rev = None
        initial = {
            "content": ("Describe your new page %s here...") % slug,
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if not rev_id and initial["content"] == form.cleaned_data["content"]:
                form.errors["content"] = (("You have made no changes!"),)
